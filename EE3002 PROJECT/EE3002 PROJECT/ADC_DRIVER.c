...
void adc_multi_ch_init(char adc, char channels, char * adc_channels)
{
	...
	for(i=0;i< channels;i++)
	{
		if(adc_channels[i]<8)
		{
			gpio_init(PA,adc_channels[i],IN,I_A);
		}
		else if(adc_channels[i]<10)
		{
			gpio_init(PB,adc_channels[i]-8,IN,I_A);
		}
		else if(adc_channels[i]<16)
		{
			gpio_init(PC,adc_channels[i]-10,IN,I_A);
		}
	}
	if(adc == 1)
	{
	...
	while(1)
		{
			if(adc_check(adc1))
			{
				temp_rx = ADC1->DR;
				analog_rx[i] = temp_rx;
				//analog_rx[i] = (pow(1.0022,(temp_rx))) * 5;
				i++;
				if(i == channels)
				{
					i = 0;
